...
export default function SearchPage({
  ...
  const results = trimmedQuery
    ? allMockProducts.filter((product) =>
        ...
        (product.description && product.description.toLowerCase().includes(trimmedQuery.toLowerCase()))
      )
    : [];
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 bg-gray-50/50 min-h-screen">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">
          {trimmedQuery
            ? `${t("Search Results for")} "${trimmedQuery}"`
            : t("Search Products")}
        </h1>
        <p className="text-gray-500">
          {trimmedQuery
            ? `${t("Found")} ${results.length} ${t("product")}${results.length !== 1 ? 's' : ''}`
            : t("Enter a search term to find products")}
        </p>
      </div>
      ...
      {trimmedQuery && results.length > 0 ? (
        ...
        <div className="py-20 text-center">
          <p className="text-gray-500 text-lg">{t("Enter a search term to find products.")}</p>
        </div>
      )}
    </div>
  );

...
function StarRating({ rating = 0, reviewCount }: { rating?: number; reviewCount?: number }) {
  ...
  for (let i = 0; i < 5; i++) {
    if (i < fullStars) {
      ...
      stars.push(
        <svg key={i} className="w-3 h-3 text-amber-400" viewBox="0 0 20 20">
          <defs>
      ...
      stars.push(
        <svg key={i} className="w-3 h-3 text-gray-300 fill-gray-300" viewBox="0 0 20 20">
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      );
    }
    ...
    <div className={`bg-white rounded-xl border border-gray-100 overflow-hidden group hover:shadow-lg transition-all duration-300 flex flex-col ${className}`}>
      ...
      {showDealLabel && (
        <div className="flex items-center justify-between px-2.5 pt-2.5 pb-1">
          <span className="text-primary text-[10px] sm:text-xs font-semibold">{t("Deal of the Day")}</span>
          {saleEndTime && <CountdownTimer targetDate={saleEndTime} variant="premium" />}
        </div>
      )}

      {/* Product Image */}
      <Link href={`/product/${id}`} className="block relative overflow-hidden">
        <div className="relative w-full aspect-square bg-gray-50 flex items-center justify-center p-3 sm:p-4 min-h-[120px]">
          <Image
            src={imageUrl}
            alt={name}
            width={200}
      ...
      <div className="px-2.5 sm:px-3 pb-3 pt-2 flex flex-col gap-1 flex-1">
        ...
        {description && (
          <p className="text-[10px] sm:text-xs text-gray-400 line-clamp-1">{t(description)}</p>
        )}
